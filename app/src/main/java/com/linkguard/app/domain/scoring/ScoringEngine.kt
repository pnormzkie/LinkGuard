...
class ScoringEngine {
    ...
    fun evaluate(signals: List<ScanSignal>, externalCoverageMissing: Boolean = false): ScanVerdict {
        ...
        val verdict = when {
            signals.any { it.strength == SignalStrength.CRITICAL } || totalScore >= SCORE_THREAT_THRESHOLD -> Verdict.THREAT
            signals.any { it.strength == SignalStrength.STRONG } || totalScore >= SCORE_SUSPICIOUS_THRESHOLD -> Verdict.SUSPICIOUS
            else -> Verdict.SAFE
        }

        // Simplify category labeling as requested
        val sources = signals.map { it.source }.distinct()
        val primaryReason = if (verdict == Verdict.SAFE) {
            "No risks detected"
        } else {
            when {
                // Priority Case: Known Trackers
                signals.any { it.title.contains("Tracker", ignoreCase = true) } -> "Ad/Tracker Detected"
                ...
                else -> signals.firstOrNull()?.title ?: "Suspicious activity detected"
            }
        }

        val baseConfidence = when {
            signals.isEmpty() -> Confidence.HIGH
            signals.size > 1 || verdict == Verdict.THREAT -> Confidence.HIGH
        ...
        val secondaryReasons = buildList {
            addAll(signals.map { it.title }.distinct())
