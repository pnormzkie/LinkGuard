...
## 2026-09-22 — A probe that does not send the real client's request answers a different question
**Mistake:** I reported DomainAge as "dead in the field", then reversed that and told Norman I had over-claimed, because a direct curl to the RDAP endpoint returned 200. The reversal was wrong. My curl carried `-H 'User-Agent: LinkGuard/1.0'`; `DomainAgeProvider` sets no User-Agent at all, so OkHttp sends its own. Measured afterwards on the same URL: `okhttp/4.12.0` -> 403, absent header -> 403, any named agent -> 200. The registry behind rdap.org's redirect filters on the agent. Because the provider fails loud on any non-404, that 403 silently removed the domain-age check for every user on every .com lookup — the single best signal against freshly registered look-alike domains, which is this app's main threat. I had the right answer, disproved it with a probe that was not the app, and talked myself out of it with more confidence than the first claim.
